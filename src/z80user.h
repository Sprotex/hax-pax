...
extern volatile int fdcsync; // variable for syncing with NOP injection...

// mapon - if mapping is enabled
// pfdon - enables PFD (Pseudo-floppy-disk) for non-M1 reads/writes
...
{                                                                       \
            (x) = memory[address] | (memory[address+1]<<8); \
}
...
{                                                                       \
  Z80_WRITE_BYTE((address),((x)&0xFF));    Z80_WRITE_BYTE((address+1),((x)>>8)); \
}
...
#define Z80_INPUT_BYTE_A(port, x)                                         \
{ \
  x = zxin(port | ((A)<<8) );                                                                       \
}
